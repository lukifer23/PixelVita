...
class ModelRepositoryImpl @Inject constructor(
    ...
    companion object {
        private const val COMPONENT = "ModelRepositoryImpl"
        private const val ASSET_MODEL_PATH = "models/sd35_medium_optimized"
    }

    override suspend fun getModels(): List<DiffusionModel> = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun downloadModel(model: DiffusionModel, onProgress: (Int) -> Unit) = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun deleteModel(model: DiffusionModel) = withContext(Dispatchers.IO) {
    ...
    override suspend fun getModel(id: String): DiffusionModel? = withContext(Dispatchers.IO) {
        modelDao.getModelById(id)
    }

    override suspend fun updateModel(model: DiffusionModel) = withContext(Dispatchers.IO) {
